...
export default function CavePoetryPage() {
  ...
  const startOnline = useCallback(() => {
    ...
    const next = createPoetryGame(playersRef.current[0], playersRef.current[1], nextDeckCycle());
    gameRef.current = next;
    setGame(next);
    setScreen("game");
    broadcast({ type: "state", game: next });
  ...
  const restart = useCallback(() => {
    ...
    const next = createPoetryGame(current.players[0], current.players[1], nextDeckCycle());
    commitGame(next);
  }, [commitGame, role]);
...
function PoetryTable({ game, role, perspective, roomCode, onAction, onRestart, onExit }: {
  ...
  const remaining = game.deadline
    ? Math.min(POETRY_ROUND_SECONDS, Math.max(0, Math.ceil((game.deadline - now) / 1000)))
    : POETRY_ROUND_SECONDS;
  const progress = (remaining / POETRY_ROUND_SECONDS) * 100;
    ...
    <div className="cp-table-shell">
      ...
      <div className="cp-table-grid">
        <aside className="cp-score-panel">
          <div className="cp-panel-label">Tribe score</div>
          <strong className="cp-total-score">{game.score}</strong>
          <span>{poetryResult(game.score)}</span>
...
function RoundState({ game, isPoet, role, onAction, onRestart }: {
  ...
  if (game.phase === "finished") {
    return (
      <motion.section className="cp-round-state" key="finished" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }}>
        ...
        <h2>{poetryResult(game.score)}.</h2>
        <p>You made <strong>{game.score}</strong> points together. The next game opens with a newly cycled word deck.</p>
  ...
  if (game.phase === "round-end") {
    return (
      <motion.section className="cp-round-state" key="round-end" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }}>
        ...
        <h2>{game.roundScore >= 0 ? "+" : ""}{game.roundScore} this round.</h2>
  ...
  return (
    <motion.section className="cp-round-state" key="ready" initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }}>
      ...
      <p><strong>{game.players[game.poet]}</strong> is the Poet. <strong>{game.players[game.poet === 0 ? 1 : 0]}</strong> guesses. You have 90 seconds.</p>
