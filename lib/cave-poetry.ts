...
export function buildPoetryDeck(cycle: number) {
  const deck = [...POETRY_CARDS];
  const random = seededRandom((cycle + 1) * 7919);
...
export function applyPoetryAction(
  ...
  if (action.type === "startRound" && game.phase === "ready" && isPoet) {
    return {
      ...game,
      phase: "playing",
      deadline: action.now + POETRY_ROUND_SECONDS * 1000,
      lastMove: null,
    };
  }
